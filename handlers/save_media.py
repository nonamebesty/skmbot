...
async def save_batch_media_in_channel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = f"https://skmfilestoresbot.jasurun.workers.dev?start=Shaktimaan_{str_to_b64(str(SaveMessage.id))}"
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Open Link", url=share_link)],
                 [InlineKeyboardButton("Bots Channel", url="https://telegram.me/AS_botzz"),
                  InlineKeyboardButton("Support YouTube", url="https://YouTube.com/@jasuranvideos")]]
            ),
            disable_web_page_preview=True
        )
        await bot.send_message(
...
async def save_media_in_channel(bot: Client, editable: Message, message: Message):
    try:
        ...
        share_link = f"https://skmfilestoresbot.jasurun.workers.dev?start=Shaktimaan_{str_to_b64(file_er_id)}"
        await editable.edit(
            f"**{caption} - {file_size}\n\n{share_link}**",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Open Link", url=share_link)],
